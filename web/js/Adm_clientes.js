...
function cargarClientes() {
        ...
        if (!$.fn.DataTable.isDataTable('#tablax')) {
            $('#tablax').DataTable({
                ...
                ajax: {
                    url: '../ContUsuarios?tipoc=AdministradorClientes&accion=mostrar',
                    method: 'GET',
                    dataSrc: '',
        ...
        $('#tablax').off('click', '.btn-edit').on('click', '.btn-edit', function (event) {
            ...
            $.ajax({
                url: '../ContUsuarios?tipoc=AdministradorClientes&accion=update&id=' + id,
                method: 'GET',
        ...
        $('#tablax').off('click', '.btn-delete').on('click', '.btn-delete', function (event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: '../ContUsuarios?tipoc=AdministradorClientes&accion=delete&id=' + id,
        ...
        $('#form_agg').off('submit').on('submit', function (event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: '../ContUsuarios?tipoc=AdministradorClientes&accion=agregar',
                        data: $(this).serialize(),
                        success: function (response) {
                            $('#form_agg')[0].reset();
        ...
        $('#formUpdate').off('submit').on('submit', function (event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: '../ContUsuarios?tipoc=AdministradorClientes&accion=update2',
                        data: $(this).serialize(),
                        success: function (response) {
